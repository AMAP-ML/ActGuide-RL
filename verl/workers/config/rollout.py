...
class MultiTurnConfig(BaseConfig):
    ...
    # When False, tool schemas are not passed to apply_chat_template (prompt is built from
    # messages only). Use this to align with eval where tools are embedded in system prompt
    # (e.g. DeepTraceHub). Tool map and parsing still use tool_config_path; only tokenization
    # omits the schema.
    pass_tool_schema_to_template: bool = True
