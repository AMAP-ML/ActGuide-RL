...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if "tool_call_counts" in batch.non_tensor_batch:
        tool_call_counts = np.asarray(batch.non_tensor_batch["tool_call_counts"], dtype=np.float64)
        metrics["tool_call/count_mean"] = float(np.mean(tool_call_counts))
        metrics["tool_call/count_total"] = float(np.sum(tool_call_counts))

    if "tool_call_success_flags" in batch.non_tensor_batch:
        tool_success_lists = batch.non_tensor_batch["tool_call_success_flags"]
        flat_success_flags = []
        for flags in tool_success_lists:
            flags_list = list(flags) if flags is not None else []
            flat_success_flags.extend(float(v > 0) for v in flags_list)

        total_calls = len(flat_success_flags)
        metrics["tool_call/total"] = float(total_calls)
        if total_calls > 0:
            success_arr = np.asarray(flat_success_flags, dtype=np.float64)
            metrics["tool_call_success/rate"] = float(np.mean(success_arr))
            metrics["tool_call_success/total"] = float(np.sum(success_arr))

    _REWARD_EXTRA_KEYS = {"accuracy"}
    for key in _REWARD_EXTRA_KEYS:
